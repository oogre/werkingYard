...
public void controlEvent(ControlEvent theEvent) {
  ...
  if(theEvent.isGroup()) { 
    ...
    // baud rate selected
    if(theEvent.getGroup().getName()=="BAUD") { baudrate = (int)theEvent.getGroup().getValue(); println("baud="+baudrate); return; }

    // serial port selected
    if(theEvent.getGroup().getName()=="PORT") {
      if (port != null) port.stop();

      try { port = new Serial(this, Serial.list()[(int)theEvent.getGroup().getValue()], baudrate); }
      ...
      println("port open: " + Serial.list()[(int)theEvent.getGroup().getValue()]);
    ...
    if(theEvent.getGroup().getName()=="JOG X") {
...
//      println(jog[idx.X]);
    ...
    if(theEvent.getGroup().getName()=="JOG Y") {
      i = (int)theEvent.getGroup().getValue();
...
//      println(jog[idx.Y]);
    ...
    if(theEvent.getGroup().getName()=="JOG Z") {
      i = (int)theEvent.getGroup().getValue();
...
//      send_file(file);
    ...
    if(theEvent.getController().getName() == "FEED X") { feed[idx.X] = theEvent.getController().getValue(); }
    if(theEvent.getController().getName() == "FEED Y") { feed[idx.Y] = theEvent.getController().getValue(); }
    if(theEvent.getController().getName() == "FEED Z") { feed[idx.Z] = theEvent.getController().getValue(); }
...
//        println(jog[idx.Z]);
  ...
  if (key == '[') {
    ((Toggle)cP5.getController("arc_mode")).setValue(!ArcMode); open_group(ArcMode? 'A':'J');
  }
  if (key == ']') {
...
public void update_console() {
  ...
  for (int i=0; i<idx.size(); i++) {
    ((Textlabel)cP5.getController(idx.strVal[i]+"_POS")).setVisible(DEBUG ? true : PortResponding);
    ((Textlabel)cP5.getController(idx.strVal[i]+"_POS")).setValue(idx.strVal[i]+floatCoord(position[i]));
  }
  if (port == null || SendingSequence && !Paused) { cP5.getController("GCODE").setVisible(DEBUG ? true : false); } 
  else { 
...
public void update_toggles() {
  // set visibility
  cP5.getController("sending_LED").setVisible(DEBUG ? true : PortResponding);
  cP5.getController("paused_LED").setVisible(DEBUG ? true : PortResponding);
  cP5.getController("waiting_LED").setVisible(DEBUG ? true : PortResponding);
  cP5.getController("absolute_mode").setVisible(DEBUG ? true : PortResponding);
  //cP5.getController("inch_mode").setVisible(DEBUG ? true : PortResponding);
...
public void update_port_selector() {
  ...
  ports_ddl.setVisible(DEBUG ? true : !SendingSequence);
...
public void update_func_buttons() {
  if (!PortResponding) {
    cP5.getController("FILE").setVisible(DEBUG ? true : false);
    cP5.getController("PAUSE/RESUME").setVisible(DEBUG ? true : false);
    cP5.getController("CANCEL").setVisible(DEBUG ? true : false);
    return;
  }
...
public void update_jog_buttons() {
 ...
 // if ((int)cP5.getController("mem_mode").getValue() != (MemMode? 1:0)) cP5.getController("mem_mode").setValue((MemMode? 1:0));
  ...
  for (int i = 0; i < jog_buttons.length; i++) cP5.getController(jog_buttons[i]).setVisible(DEBUG ? true : Visible);
  for (int i = 0; i < jog_toggles.length; i++) cP5.getController(jog_toggles[i]).setVisible(DEBUG ? true : Visible);
...
public void setup_setting_controls(int x, int y, int y_off) {
  ...
  tf.getCaptionLabel().getStyle().setMarginLeft(-tf.getCaptionLabel().getStyle().marginLeft);
